...
def create_cities(n):
    for i in range(n):
        try:
            city = City(title=random.choice(CITIES))
            city.save()
...
def create_trains(n):
    ...
    for i in range(n):
        try:
            ...
            train_number = random.randint(1, 999999)

            new_train = Train(number=train_number, travel_time=travel_time, from_city=from_city, to_city=to_city)
            new_train.save()
            print(f'"{new_train}" created')
